...
| `nfsMountOptions`  a| 
Control detallado de las opciones de montaje NFS; valor predeterminado es “-o nfsvers=3”. *Disponible sólo para `ontap-nas` y.. `ontap-nas-economy` controladores*. https://["Consulte la información de configuración del host NFS aquí"^].
 a| 
`-o nfsvers=4`
...
Para obtener información acerca de FlexGroups y las cargas de trabajo adecuadas para FlexGroups, consulte https://["Prácticas recomendadas y guía de implementación de los volúmenes FlexGroup de NetApp"^].
