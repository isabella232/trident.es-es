...
----
. Uso `helm install` y especifique un nombre para su implementación y ubicación del registro de imágenes. Su link:../trident-get-started/requirements.html#container-images-and-corresponding-kubernetes-versions["Imágenes Trident y CSI"] Se pueden ubicar en un registro o en diferentes registros, pero todas las imágenes CSI deben estar ubicadas en el mismo registro. En los ejemplos: `23.07.0` Es la versión de Astra Trident que está instalando.
+
[role="tabbed-block"]
====
...
|===
Por ejemplo, para cambiar el valor predeterminado de `debug`, ejecute lo siguiente `--set` comando donde `23.07.0` Es la versión de Astra Trident que está instalando:
