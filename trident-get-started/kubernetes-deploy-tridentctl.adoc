...
[role="lead"]
Puede instalar Astra Trident con `tridentctl`. Este proceso se aplica a las instalaciones en las que se encuentran las imágenes Trident y CSI en un `docker.io` y.. `registry.k8s.io` o un registro privado duplicado. Para personalizar su `tridentctl` despliegue, consulte link:kubernetes-customize-deploy-tridentctl.html["Personalice la implementación trimentctl"].
...
----
--
.Imágenes Trident y CSI en un registro duplicado
--
[listing]
----
...
----
--
.Imágenes Trident y CSI en diferentes registros reflejados
--
