...
Pulg `operator.yaml`, actualizar `image: docker.io/netapp/trident-operator:23.01.0` para reflejar la ubicación del registro de imágenes. Por ejemplo:

* `image: <your-registry>/trident-operator:23.01.0` si todas las imágenes están ubicadas en un registro.
* `image: <your-registry>/netapp/trident-operator:23.01.0` Si sus imágenes Trident y CSI se encuentran en diferentes registros.
...
=== Paso 4: Actualice la ubicación del registro de imágenes en el `TridentOrchestrator`

Sus imágenes Trident y CSI se pueden ubicar en un registro o en diferentes registros, pero todas las imágenes CSI deben estar ubicadas en el mismo registro. Actualizar `deploy/crds/tridentorchestrator_cr.yaml` para agregar las especificaciones de ubicación adicionales basadas en la configuración de su registro.
...
=== Paso 5: Cree el `TridentOrchestrator` E instale Trident
